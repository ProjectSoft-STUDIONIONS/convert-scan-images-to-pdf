...
[assembly: AssemblyDescription("${config.description} v${config.version}")]`,
	...
	compile = function() {
		return new Promise(function(resolve, reject){
			let app = `csc.exe`,
				args = [
					`/win32icon:${normalize([__dirname, "programm", "favicon.ico"])}`,
					`/resource:${normalize([__dirname, "programm", "programm.resource"])}`,
					`/out:${config.product}.exe`,
					`/win32manifest:${normalize([__dirname, "programm", "app.manifest"])}`,
					`/target:exe`,
					`${normalize([__dirname, "programm", "img_menu2pdf.cs"])}`,
					`${normalize([__dirname, "programm", "AssemblyInfo.cs"])}`
				],
				ls = spawn( app, args );
...
(async function(){
	...
	try { await unlink(`${normalize([__dirname, "programm", "app.manifest"])}`); } catch (error) { }
	try { await unlink(`${normalize([__dirname, "programm", "AssemblyInfo.cs"])}`); } catch (error) { }
	fs.writeFileSync(`${normalize([__dirname, "programm", "app.manifest"])}`, app_manifest, {encoding: "utf8"});
	fs.writeFileSync(`${normalize([__dirname, "programm", "AssemblyInfo.cs"])}`, assemblyinfo_cs, {encoding: "utf8"});
